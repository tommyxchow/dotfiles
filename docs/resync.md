...
It links configs and first-party skills, prunes known stale paths, writes `~/.claude/statusline-command.sh`, and copies Cursor's local `tc` plugin. The official `gh` skill is not an installer link; see Official gh skill below. It also seeds `~/.grok/config.toml` from `grok/config.toml` on new machines and patches only that file's non-default keys on re-runs — Grok writes runtime state into it, so it is never symlinked. Same for `~/.grok/lsp.json` (seed if missing, warn if `typescript-language-server` is not on PATH; never overwrite an existing file). Re-running is safe. This is the step that makes Claude / Cursor / Grok / OpenCode 2 pick up instructions and `vet` / `tldr` / `polish` / `review` / `tdd` / `grill-me` / `refresh` / `pass` / `pr` / `cleanup` on a new machine.
...
Run the `cleanup` skill in each candidate repo (the sibling project folders from "Find the repo"). It prunes worktree registrations whose folder is already gone, which nothing else does on its own, and reports dead branches rather than deleting them. Grok is the one harness that never sweeps its own worktree folders, so also run `grok worktree gc --max-age 7d` where Grok is installed; Claude Code and Cursor sweep theirs on a timer.

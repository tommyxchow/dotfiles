...
5. **Fix in one batch.** Root cause, not the line the bot pointed at; test for the corrected behavior where testable; check related in-scope paths for the same mistake. Close the batch through `pass`, then follow the global completion rule on the changed code, reusing unaffected evidence. One commit, `fix(<scope>): address review` with the threads' subjects in the body. One push, then refresh the body against that head, not one push per comment, then watch CI as Open step 7 says, with no second bot wait.

...
7. **Learn.** If a thread class recurred, or a bot found something `review` should have caught, propose one line in the report for that repo's `REVIEW.md` when it has one, and otherwise for the review section of its `AGENTS.md`. Propose it rather than applying it, because that file is team-shared and outside the ticket.

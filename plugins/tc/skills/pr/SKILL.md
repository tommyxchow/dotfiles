...
7. **Watch CI.** Run `gh pr checks <number> --watch --fail-fast` in the background with a deadline, since a run can outlast a tool call, and act as soon as it returns. Exit 0 is green. Under `--watch` the command returns only once nothing is pending, so a deadline that fires first means still pending, which is not green; say so. On a failure, read the failed job's log with `gh run view <run-id> --log-failed`, reproduce it locally, fix through the normal loop, and push once; a second red on the same check is the stop, one push rather than the global budget's three because each CI round costs minutes. The text `no checks reported on the '<branch>' branch` means the repo has no CI, which the report says instead of claiming green. Then, in a repo with push-triggered review bots, poll `gh pr view <number> --json reviews,comments` about once a minute for up to ten minutes until the first bot review lands, then run Update once and notify; no bots means no wait, and a bot that has not posted by then is noted in the report. `gh run` needs a normal `gh auth login`, not a fine-grained token.

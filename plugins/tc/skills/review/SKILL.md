...
`$ARGUMENTS`: an optional depth first (`quick`, or `deep` / `deeper` / `thorough` for the expensive tier), then an optional scope keyword (`staged`, `unstaged`, `branch`, `all`, `pr <number|url>`), an optional `fix`, then focus text. Bare `review` uses the default scope and reports only.

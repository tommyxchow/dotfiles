---
name: polish
description: Review changed code across four parallel lenses — reuse, quality, efficiency, altitude — then apply the high-confidence cleanups. Quality only; it does not hunt correctness bugs (that is /code-review's job). Use when asked to "polish", "polish this", "clean this up", "tidy this", "refactor this", "dry clean", "reduce duplication", "make this less hacky", or for a final cleanup pass over current changes before committing. Reviews the union of git changes and files edited this session, across repos.

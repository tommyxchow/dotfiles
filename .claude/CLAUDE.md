...
- **IMPORTANT: readable beats brief.** Other instructions may tell you to keep answers to a few lines or skip explanation. Apply that to tool output and code, not to what you write to me. Be short by saying fewer things, never by squeezing sentences into fragments or notes.
...
- No labels standing in for sentences. Write "The tests pass" and "The build is still red", not "Outcome: tests pass" or "Status: build red". No arrow chains like `A → B → C` in place of a sentence.
...
- Use one name for one thing. Don't coin nicknames or shorthand for things that already have names, and don't echo my shorthand back at me as if it were a term.
...
I usually auto-accept and only read when you finish. Sometimes I scroll back to one step. Write so the closing message is enough on its own, and so any single step still makes sense if I jump to it.
...
- Mid-run, write only at milestones: you found something that changes the plan, you changed direction, or you hit a wall. One or two sentences each. Routine progress needs no message; the close covers it. Anything you do write stands alone like the close: full sentences, no dumps of tool output.
- Before a long or open-ended run, write down in a few checkable lines what you will treat as done, work against that, and report against it at the end.
...
- We good, anything outstanding, quick pass, do a pass, final pass, final review, final double check, close this out, or "pass" on its own: follow the `pass` skill. The word mid-sentence (tests pass, pass a prop) never triggers it. Pass is not polish, not a code review, and not permission to commit.
- Double check or verify: route by what I'm pointing at. A claim or current docs is `vet`. Whether the code is correct is code review or Bugbot. A finished slice of work is `pass`.

...
- **Planning and building in one model is the normal case; a plan does not need a different model or a new session to build it.** When work does move to another model or session, the plan carries everything the builder needs: approved scope, decisions, constraints, code entry points, checks, and current progress. It also carries its stop conditions: the code no longer matches the plan, a check fails twice, or the fix needs a file the plan put out of scope. On any of those the builder stops and reports instead of improvising. The builder owns implementation details.
...
- **Inside a herdr pane (`HERDR_ENV` is set), use herdr for what outlives the session.** Load the `herdr` skill for the exact commands; its description says to wait until I mention herdr, and this rule is that mention. A dev server, a watcher, or anything I should be able to watch runs in a sibling pane, split from the current one and started with `herdr pane run`, never as a background job of the harness. Before starting a server, list the panes and reuse one that already runs it for this checkout. Wait for its ready line with `herdr pane wait-output` before driving the app, and read that pane when a request fails. When you created a herdr worktree from another pane, its servers and its next session go in the worktree's own workspace, starting with the first pane `herdr worktree create` returned. When the fresh-session split above says to stop, start the next session yourself: split a pane, start an agent of this same harness in it on the model this session runs on, prompt it with the plan file path and the slice to build, then finish this session's report. Outside herdr, the harness's own background tool and a plain handoff message stand in.
- **In herdr, a session keeps its own names current.** At the start of a task, check the names of what you occupy or created, fix what no longer fits, and leave what already does. Name your tab after the task with `herdr tab rename`, a lowercase slug of two or three words and at most 16 characters, like `nav-flicker`, because the sidebar clips longer ones. Do it when you are the only agent in the tab and its label is a number or a slug: numbers come from herdr and slugs from agents. Any other label is one I typed, so it stays, and a name I ask you to keep gets a capital letter for that reason. Leave your own agent name alone, since it tells me which harness is running; an agent you start gets a name for its role, like `reviewer`. Label each pane you split with `herdr pane rename`, by what it runs and the port it serves, like `dev :3001`, so I and other sessions can see which ports are taken. Workspaces keep their folder or branch name, so keep a worktree's branch name short too. When a tab already holds three panes, the next one goes in a new tab with a slug for what it holds. Close a pane you created once its job is over, like a finished test run; a dev server stays up under its label.
...
- **Pick the checkout by the task.** A session I opened in a worktree stays there; never nest another. From the main checkout, a small change stays put, on `main` or a `tc/` branch as the route needs. Larger or longer work, or several slices, gets its own worktree so other sessions in this repo keep working undisturbed: inside a herdr pane (`HERDR_ENV` is set) create it with `herdr worktree create --cwd <this checkout> --branch <name> --no-focus`, which groups it in the sidebar and copies its env files; without `--cwd` herdr branches from the workspace I have open, which may be another repo. Elsewhere use the harness's own worktree command. Never `git worktree add` into a folder I didn't open. Then continue in that checkout. A harness with neither branches in place and says so.

...
- **Walk me through the change in the close, sized to it.** A small change shows its whole hunk. A bigger one shows the one to four hunks that carry the idea, a new condition, a permission check, a tricky query, a decision you made in code, in the order the data flows, each with one plain line above it saying what it does, then the scope (files touched, roughly how much) and the one command that shows the full diff. Never paste a big change's whole diff. None of this for a mechanical change.
...
- Use the same model to plan and build unless I choose otherwise. When work moves to another model or session, the existing plan carries approved scope, decisions, constraints, code entry points, checks, current progress, and its stop conditions: the code no longer matches what the plan describes, a check fails twice, or the fix needs a file the plan put out of scope. On any of those the builder stops and reports instead of improvising. The builder owns implementation details; ask again only when new evidence changes an agreed decision, scope, or risk.
...
- **Where the harness keeps memory across sessions**, a note is a correction I gave or an approach that proved out, with the one-line why. Not what the repo, git history, or these files already say. Short and specific beats long, since an index line or a search hit is all a later session sees. Update an existing note rather than add a twin, and delete one proven wrong.
...
- Validate external input at the boundary with the repo's validator, then trust the types. Prefer Zod when choosing a TS validator; adding it still needs dependency approval. Security stays on the server. Never interpolate untrusted input into a shell command, query, filesystem path, or outbound URL, and no secrets in `PUBLIC` env vars or the client.
...
- Browser UAT is opt-in; `ship it` counts as yes. Use the session's browser tool in your own tab; when it connects to shared Chrome, coordinate access and leave my tabs alone. If opted-in UAT is unavailable or errors, report the blocker and pause that check while finishing independent work.
...
- **Pick the checkout by the task and say which.** A session I opened in a worktree stays there; never nest another. From the main checkout, a small change stays put, on `main` or a `tc/` branch as the route needs. Larger or longer work, or several slices, gets its own worktree so other sessions in this repo keep working undisturbed: inside a herdr pane (`HERDR_ENV` is set) create it with `herdr worktree create --branch <name>`, which groups it in the sidebar and copies its env files; elsewhere use the harness's own worktree command. Never `git worktree add` into a folder I didn't open. Then continue in that checkout. A harness without either branches in place and says so.

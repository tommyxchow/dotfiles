...
- When a decision or approval needs my answer, prefer the question tool actually exposed by this session. If you are about to write options into a message, that is the tool's job. Use its supported fields and limits, not another harness's tool name or schema.
- Shape a round so I can answer it fast: a short title on each question, the recommended option first and marked `(Recommended)` with a short reason, multiple selection only when the choices can combine, and independent questions asked together rather than one at a time. Split into another round only when the tool cannot hold them, or when one answer changes what the rest should be. Where the tool renders a preview, put the thing being chosen in it, a mockup, a snippet, or the rendered line, so I pick by looking instead of imagining.
- If that tool is absent or unavailable in this mode, ask in text: a clear question followed by `- [1] Option (recommended)` and `- [2] Other option`, with a short reason for the recommendation. For several questions, give each its own options so I can answer `Q1: 1, Q2: 2`. Use the same shape for text approvals and for the Next block below, which stays text even where the question tool exists. Wait for my answer before the dependent action; a dismissed, failed, timed-out, or unanswered tool call is not a decision or approval.

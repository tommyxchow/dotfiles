...
- **Pick the checkout by the task and say which.** A session I opened in a worktree stays there; never nest another. From the main checkout, a small change stays put, on `main` or a `tc/` branch as the route needs. Larger or longer work, or several slices, gets its own worktree so other sessions in this repo keep working undisturbed: use the harness's own worktree command, never `git worktree add` into a folder I didn't open, then continue in that checkout. A harness without one branches in place and says so.

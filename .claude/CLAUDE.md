...
- **A build ends with `pr`, not a bare commit.** It proves the checklist, runs `review branch fix` in a fresh subagent, runs `pass`, pushes, and opens the draft. It runs once near the end, since work bots review every push to a draft. Once the PR exists, its body is yours: any commit that changes what the PR does or its evidence refreshes it in the same step.

...
- **Rule-following decays as a session runs long, not as this file grows.** A long session keeps the task and loses the conventions. So in long work, write the decisions made so far into the plan at a slice boundary and carry on in a fresh session, which reads the plan and follows these rules better than a long one that remembers the conversation. Don't split a session that is still short.

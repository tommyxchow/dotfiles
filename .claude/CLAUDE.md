...
- Validate external input at the boundary with the repo's validator, then trust the types. Prefer Zod when choosing a TS validator. Security stays on the server. Never interpolate untrusted input into a shell command, query, filesystem path, or outbound URL, and no secrets in `PUBLIC` env vars or the client. Don't hand-roll auth, sessions, or crypto; use what the platform or repo already has.

...
- **The completion rule is the same with or without a PR.** Account for each acceptance criterion, run the local check, and run `review all` on the complete task diff, including pending and untracked work. Then run `pass`, which applies the confirmed in-scope findings.

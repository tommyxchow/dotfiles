...
- **Pick the checkout by the task and say which.** A session I opened in a worktree stays there; never nest another. From the main checkout, a small change stays put, on `main` or a `tc/` branch as the route needs. Larger or longer work, or several slices, gets its own worktree so other sessions in this repo keep working undisturbed: inside a herdr pane (`HERDR_ENV` is set) create it with `herdr worktree create --branch <name>`, which groups it in the sidebar and bootstraps it; elsewhere use the harness's own worktree command. Never `git worktree add` into a folder I didn't open. Then continue in that checkout. A harness without either branches in place and says so.
...
- A worktree is a clean checkout of tracked files only, so gitignored ones like `.env` don't come along, and it isolates files but not ports or local databases. In a fresh one, install dependencies and copy the env files over from the main checkout before the first check, and say so. Herdr's bootstrap plugin does both for a worktree under `~/.herdr/worktrees`, so there check for the env files and installed dependencies first and only do what is missing. Assume other sessions of mine are running in sibling checkouts of the same repo: don't switch branches, stash, or rewrite a ref another session could be using, and give any server or database you start its own port.

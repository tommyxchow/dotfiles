...
- **Open a task by naming its route in one line**: straight to main, a PR, a stacked PR, or a mechanical loop, and which steps it skips and why. That first line is where I catch a mis-sizing.
...
- **Verify it where a user would meet it before `pass` closes the slice.** Green tests are the floor. Then drive the change in the most realistic place the harness can reach: a page in the browser against the local dev server, a route with a real request, a CLI by running it, using the harness's own app-running or browser tooling where it has one. Anything off gets fixed and driven again until the checklist holds; a fix that changes behavior goes back through `tdd`, a visual fix goes straight in. Say what was driven and what only tests cover, since `pr` cites that split as evidence. The browser part follows the UI opt-in rule; the rest needs no opt-in.
- **`ship it` runs the chain to a draft PR without another check-in.** Said once the plan is approved, or on work small enough not to need one, it means build with `tdd`, verify and loop, `pass` each slice, then `pr`, which owns the completion rule and the draft from there. It counts as the yes to browser UAT, and the route is a draft PR even where the Git rules would allow `main`, so on the default branch a `tc/` branch comes first. It skips none of my checkpoints: the plan gate when scope needs a plan and there is none, a decision only I can make, marking ready, and merging. The fresh-session split still applies; the plan then records that `ship it` is in progress so the next session resumes the loop.

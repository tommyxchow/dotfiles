...
- **Preflight before the first edit of `ship it` or any planned task.** Record the starting commit, run the repo's full check on the untouched tree, and read the default CI workflow once so the local check covers what CI runs. Then start what the checklist's checks need that stays process-local: the dev server on its own port, an MCP client for a server this task is building, `gh`. A browser, simulator, emulator, or phone starts only after `uat`.
...
- In herdr, a dev server, a watcher, or anything I should be able to watch runs in a sibling pane, never as a background job of the harness. Split the pane from the current one with `herdr pane split --current --no-focus` and start the process with `herdr pane run`. Before starting a server, list the panes and reuse one that already runs it for this checkout.
...
- **The completion rule is the same with or without a PR.** Account for each acceptance criterion, run the repo's full check, and run `review all` on the complete task diff, including pending and untracked work. Then run `pass`, which applies the confirmed in-scope findings.
...
- In a repo of mine, a finished task may also push its own branch or `main` when completion holds cleanly: every criterion accounted for, the repo's own full check green, and no confirmed review finding left unfixed.
...
- The repo's full check is its own check script when it has one; otherwise it is the repo's typecheck, lint, and tests together, each run with the repo's own command. Prefer that over a single linter pass, and don't invent a gate the repo doesn't have.
